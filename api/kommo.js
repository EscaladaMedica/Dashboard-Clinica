...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(url, {
      method: 'GET',
      headers: {
        // Token de longa duração do Kommo — funciona como Bearer direto
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
        // Header extra exigido pela API v4 do Kommo em alguns planos
        'X-Auth-Token': token,
      },
    });

    // Log do status para debug nos logs do Vercel
